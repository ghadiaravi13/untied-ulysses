...
DUMP_FOLDER=${DUMP_FOLDER:-"${ROOT_TMP}/run_all_multinode_32B"}
BASE_TRACE_FOLDER="run_all_multinode_32B"
MEM_BASE_TRACE_FOLDER="run_all_multinode_32B"
...
WANDB_PROJECT=${WANDB_PROJECT:-"run_all_multinode_32B"}
...
METHODS=(
    "upipe upipe_fa3_offload_tiled_mlp 8 2"
    "orig_usp_ulysses usp_fa3_offload_tiled_mlp 8 2"
    "orig_usp_ring usp_fa3_offload_tiled_mlp 1 16"
    "torch_ring torch_ring_alltoall 1 16"
)
...
for method_config in "${METHODS[@]}"; do
    ...
    for seqlen in "${SEQLENS[@]}"; do
        ...
        # Adjust PIN_MEMORY for very long sequences >3M
        if [ "${seqlen}" -gt 3145728 ]; then
