#!/usr/bin/bash
# Multi-experiment runner for context-parallelism configurations (multinode).
# Keeps all functional behavior while improving readability.
...
DUMP_FOLDER=${DUMP_FOLDER:-"${ROOT_TMP}/deterministic_loss_traces_8b_c4"}
BASE_TRACE_FOLDER="deterministic_loss_traces_8b_c4"
MEM_BASE_TRACE_FOLDER="deterministic_loss_traces_8b_c4"

CONTEXT_LENGTH=${CONTEXT_LENGTH:-"524288"} # 512K
PROFILING_SUFFIX=""
...
OFFLOADING=${OFFLOADING:-"cpu"} # ["no", "cpu", "UAO", "TAO"] Unsloth/Torchtune offloading
...
WANDB_PROJECT=${WANDB_PROJECT:-"loss_traces_8b"}
...
export PIN_MEMORY=${PIN_MEMORY:-"False"}
export INP_PIN_MEMORY=${INP_PIN_MEMORY:-"False"}

# no clear cuda cache
export CLEAR_CUDA_CACHE=${CLEAR_CUDA_CACHE:-"0"}
export MEMORY_SNAPSHOT_MAX_ENTRIES=${MEMORY_SNAPSHOT_MAX_ENTRIES:-"10000000"}
...
trace_folder_name() {
    ...
    echo "${BASE_TRACE_FOLDER}_$((CONTEXT_LENGTH / 1024))k_ACS_${AC_LAYER_STRIDE}_BS_${BATCH_SIZE}_${ulysses_degree}u${ring_degree}r_${PROFILING_SUFFIX}_${ring_comm_heads}_${attn_impl}_ao_${OFFLOADING}_ac_${CHECKPOINTING}_fsdpoffl_${FSDP_OFFLOADING}_compile_${COMPILE}_deterministic_${DETERMINISTIC}"
}
...
mem_trace_folder_name() {
    ...
    echo "${MEM_BASE_TRACE_FOLDER}_$((CONTEXT_LENGTH / 1024))k_ACS_${AC_LAYER_STRIDE}_BS_${BATCH_SIZE}_${ulysses_degree}u${ring_degree}r_${PROFILING_SUFFIX}_${ring_comm_heads}_${attn_impl}_ao_${OFFLOADING}_ac_${CHECKPOINTING}_fsdpoffl_${FSDP_OFFLOADING}_compile_${COMPILE}_deterministic_${DETERMINISTIC}"
}
...
should_skip_config() {
    ...
    # 1u8r (pure ring) only allowed for usp or torch (have ring support)
    # if [ "${config}" = "1 8" ] && [[ "${attn_impl}" != *"usp"* ]] && [[ "${attn_impl}" != *"torch"* ]]; then
    #     return 0
    # fi

    # 8u1r doesn't allow torch_ring_alltoall
    if [ "${config}" = "8 1" ] && [ "${attn_impl}" = "torch_ring_alltoall" ]; then
        return 0
    fi
...
build_overrides() {
    ...
    if [ "${DETERMINISTIC}" = "true" ]; then
        overrides+=(--training.deterministic)
    fi
}
...
# Derived settings
################################################################################
if [ "${CONTEXT_LENGTH}" -gt 4194304 ]; then
    INP_PIN_MEMORY="False"
fi

total_experiments=$((${#EXPERIMENTS[@]} * ${#ATTN_IMPL_OPTIONS[@]} * ${#RING_COMM_HEADS_OPTIONS[@]}))

echo "Starting multi-experiment multinode run with ${total_experiments} configurations..."
...
for config in "${EXPERIMENTS[@]}"; do
    ...
    for attn_impl in "${ATTN_IMPL_OPTIONS[@]}"; do
        for ring_comm_heads in "${RING_COMM_HEADS_OPTIONS[@]}"; do
            ...
            cp -f "$(get_script_path)" "${DUMP_FOLDER}/${trace_folder}/run_all.sh"
