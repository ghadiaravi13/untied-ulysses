...
DUMP_FOLDER=${DUMP_FOLDER:-"${ROOT_TMP}/batch_size_run_all_8b_SAC"}
BASE_TRACE_FOLDER="batch_size_run_all_8b_SAC"
MEM_BASE_TRACE_FOLDER="batch_size_run_all_8b_SAC"
...
WANDB_PROJECT=${WANDB_PROJECT:-"batch_size_run_all_8b_SAC"}
...
export AC_LAYER_STRIDE=${AC_LAYER_STRIDE:-"7"}
...
METHODS=(
    ...
    "upipe_SAC_SAO upipe_fa3_offload_tiled_mlp 8 1"
    "usp-ulysses_SAC usp_fa3_offload_tiled_mlp 8 1"
...
for method_config in "${METHODS[@]}"; do
    ...
    for ((si=0; si<${#SEQLENS[@]}; si++)); do
        ...
        # see Untied-Ulysses/torchtitan/patch_torch_files/checkpoint_wrapper.py
        # setting this to K will skip CPU offloading of the input to every Kth layer
        if [[ "${method_name}" == "upipe_SAC_SAO" ]]; then
            export AC_LAYER_STRIDE="7"
        else
            export AC_LAYER_STRIDE="1000"
        fi
