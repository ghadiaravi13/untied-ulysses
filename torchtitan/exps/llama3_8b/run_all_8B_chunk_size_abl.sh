...
# RDZV_ID=${RDZV_ID:-"101"}
RDZV_ID=${RDZV_ID:-$(date +%s%N | tail -c 6)}
...
DUMP_FOLDER=${DUMP_FOLDER:-"${ROOT_TMP}/v2_rebuttal_chunk_size_abl_8b"}
BASE_TRACE_FOLDER="v2_rebuttal_chunk_size_abl_8b"
MEM_BASE_TRACE_FOLDER="v2_rebuttal_chunk_size_abl_8b"

CHUNK_SIZE=${CHUNK_SIZE:-"1"}

PROFILING_SUFFIX="chunk_size_${CHUNK_SIZE}"
COMPILE=${COMPILE:-"false"}
FLAVOR=${FLAVOR:-"8B"}

# AC/Offloading parameters
OFFLOADING=${OFFLOADING:-"cpu"} # ["no", "UAO", "TAO", "cpu"] Unsloth/Torchtune offloading
...
WANDB_PROJECT=${WANDB_PROJECT:-"v2_rebuttal_chunk_size_abl_8b"}
...
METHODS=(
    # "upipe_cs_${CHUNK_SIZE} upipe_fa3_offload_tiled_mlp 4 1"
    "usp-ulysses usp_fa3_offload_tiled_mlp 4 1"
...
run_experiment() {
    local rdzv_id="${RDZV_ID}_$(date +%s%N | tail -c 10)"
...
for method_config in "${METHODS[@]}"; do
    ...
    for seqlen in "${SEQLENS[@]}"; do
        ...
        echo ""
        echo "========================================"
        echo "[${experiment_counter}/${total_experiments}] ${method_name} @ ${seqlen_k}K"
        echo "========================================"
        ...
        export WANDB_TAGS="benchmark,${method_name},seq_${seqlen_k}k,${ulysses_degree}u${ring_degree}r"
