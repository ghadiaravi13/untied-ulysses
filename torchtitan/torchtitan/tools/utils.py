...
from torchtitan.tools.logging import logger, rank_print
...
def debug_memory_on_rank(rank: int = 0, label: str = "") -> None:
    """Helper function to print GPU memory usage and break on specified rank.

    Args:
        rank: The rank to break on. Defaults to 0.
        label: Label to identify the debug point.
    """
    pass
...
def debug_on_rank(rank: int = 0, label: str = "") -> None:
    """Helper function to break on specified rank.
    ...
    """
    if torch.distributed.get_rank() == rank:
        rank_print(f"Breaking on rank {rank} at {label}", rank=rank)
        breakpoint()


def print_memory_on_rank(rank: int = 0, label: str = "") -> None:
    debug_memory_on_rank(rank, label)
