    ...
    def state_dict(self) -> dict[str, Any]:
        try:
            func = functools.partial(
                get_optimizer_state_dict,
                # Remove flatten_optimizer_state_dict=True to avoid KeyError issue
            ...
            # Fallback: use basic optimizer state_dict if distributed version fails
            print(
                f"Warning: Distributed optimizer state_dict failed with {e}, falling back to basic state_dict"
            )
            return {
    ...
    def load_state_dict(self, state_dict: dict[str, Any]) -> None:
        try:
            func = functools.partial(
                ...
                # Remove flatten_optimizer_state_dict=True to be consistent with state_dict
                # options=StateDictOptions(flatten_optimizer_state_dict=True),
            )
            list(map(func, self.model_parts, self.optimizers))
        except (KeyError, RuntimeError) as e:
            # Fallback: check if state_dict is in basic format (optimizer_0, optimizer_1, etc.)
            print(
                f"Warning: Distributed optimizer load_state_dict failed with {e}, trying fallback format"
            )
            for i, opt in enumerate(self.optimizers):
                opt_key = f"optimizer_{i}"
                if opt_key in state_dict:
                    opt.load_state_dict(state_dict[opt_key])
                else:
                    print(f"Warning: Could not find state for optimizer {i}, skipping")
                    break
