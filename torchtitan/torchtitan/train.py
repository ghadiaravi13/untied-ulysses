...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def __init__(self, job_config: JobConfig):
        ...
        from yunchang import patch_ulysses_ring_pg, set_seq_parallel_pg
        ...
        if parallel_dims.cp_ulysses > 1:
            patch_ulysses_ring_pg(
                world_mesh["cp_ring", "cp_ulysses"].get_group(1),  # cp_ulysses = dim 1 (inner, intra-node)
                world_mesh["cp_ring", "cp_ulysses"].get_group(0),  # cp_ring = dim 0 (outer, cross-node)
            )
            # if "dist" in job_config.model.attn_impl:
            #     initialize_distributed(ring_pg=world_mesh["cp_ring", "cp_ulysses"].get_group(0))
        else:
        ...
        from torchtitan.distributed.context_parallel_2d import (
            _context_parallel_buffers_2d,
        )

        freqs_cis_buffers = [m.freqs_cis for m in self.model_parts]
        freqs_cis_buffer_seq_dims = [0 for _ in self.model_parts]
        cp_mesh = (
            world_mesh["cp_ring", "cp_ulysses"]
            if parallel_dims.cp_ulysses > 1
            else world_mesh["cp_ring"]
        )
        chunks = _context_parallel_buffers_2d(
            cp_mesh, freqs_cis_buffers, freqs_cis_buffer_seq_dims
        )
    ...
    def train_step(self, input_dict: dict[str, torch.Tensor], labels: torch.Tensor):
        ...
        #             loss.data.zero_()
            ...
            with self.train_context(optional_context_parallel_ctx):
                ...
                torch.distributed.all_reduce(
                    non_masked_tokens_total,
                    op=torch.distributed.ReduceOp.SUM,
                    group=world_mesh.get_group(mesh_dim="dp_shard_cp"),
                )  # Modifies in-place
                non_masked_tokens_total = max(
                    non_masked_tokens_total.item(), 1
        ...
        if self.job_config.evaluation.enable_eval and (
            self.step % self.job_config.evaluation.eval_freq == 0 or self.step == 1
        ):
    ...
    def validate_step(self, input_dict: dict[str, torch.Tensor], labels: torch.Tensor):
        ...
        with self.train_context(optional_context_parallel_ctx), torch.no_grad():
            assert (
                len(model_parts) == 1
            ), "Only one model part is supported for validation"
            model_parts[0].eval()
            ...
            torch.distributed.all_reduce(
                non_masked_tokens_total,
                op=torch.distributed.ReduceOp.SUM,
                group=world_mesh.get_group(mesh_dim="dp_cp"),
            )  # Modifies in-place
            non_masked_tokens_total = max(
                non_masked_tokens_total.item(), 1
            )  # Ensure no division by zero
...
def load_pretrained_checkpoint(
    ...
    if not checkpoint_path:
        logger.warning(
            "No pretrained checkpoint path provided. Skipping pretrained loading."
        )
        return
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "transformers library not installed. Please install it to load HuggingFace checkpoints"
            )
            return
        ...
        if model_args is None:
            logger.warning(
                "model_args not provided, attempting to infer from HF config"
            )
        ...
        if len(model_parts) == 1:
            ...
            missing_keys, unexpected_keys = model_parts[0].load_state_dict(
                converted_state_dict, strict=False
            )

            if missing_keys:
                logger.warning(
                    f"Missing keys when loading pretrained weights ({len(missing_keys)} total):"
                )
            ...
            if unexpected_keys:
                logger.warning(
                    f"Unexpected keys when loading pretrained weights ({len(unexpected_keys)} total):"
                )
            ...
            logger.info(
                f"Successfully loaded {len(converted_state_dict) - len(missing_keys)} parameters"
            )
            ...
            for i, model_part in enumerate(model_parts):
                missing_keys, unexpected_keys = model_part.load_state_dict(
                    converted_state_dict, strict=False
                )
                logger.info(
                    f"Loaded pretrained weights for model part {i} "
                    f"({len(converted_state_dict) - len(missing_keys)} parameters loaded)"
                )

        logger.info(
            "Successfully loaded pretrained weights from HuggingFace checkpoint"
        )
...
if __name__ == "__main__":
    ...
    init_logger(
        save_traces_folder=config.profiling.save_traces_folder
