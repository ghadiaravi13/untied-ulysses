...
class Model:
    ...
    """
    Path to pretrained model checkpoint to load from.
    If provided, will load weights from this checkpoint instead of initializing from scratch.
    Supports both .pth and HuggingFace checkpoint formats.
    """

    attn_impl: str = "torch_ring"
    """
    Default implementation is torch_ring (Torch SDPA).
    Other options are:
    - "yunchang": Yunchang USP
    - "dist_flash_attn": DistFlashAttn
    """

    ring_comm_heads: str = "mha_kv"
    """
    Default is mha_kv. Decides if ring comm is for full heads or group heads
    Other options are:
    - "gqa_kv": Grouped Query Attention with KV heads ring comm
    """
...
class Parallelism:
    ...
    context_parallel_ulysses_degree: int = 1
    """Ulysses-style context parallelism degree for sequence dimension sharding. 1 means disabled."""
...
class ActivationCheckpoint:
    mode: Literal[
        "selective", "sync_selective", "full", "none", "async_selective"
    ] = "selective"
...
class Evaluation:
    ...
    """Whether to enable periodic evaluation during training"""

    eval_freq: int = 500
    """Frequency of evaluation sampling during training (in steps)"""

    max_new_tokens: int = 100
    """Maximum number of new tokens to generate during evaluation"""

    num_eval_samples: int = 5
    """Number of fixed samples to use for evaluation"""
