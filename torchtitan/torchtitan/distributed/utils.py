...
# from patch_torch_files import _attention as attention_patch
# torch.distributed.tensor.experimental._attention = attention_patch
...
def create_context_parallel_ctx(
    ...
    cp_impl: str = "yunchang",
):
    try:
        from patch_torch_files._attention import set_load_balance, set_rotate_method

        from torchtitan.distributed.context_parallel_2d import (
            set_load_balance_2d,
            set_rotate_method_2d,
        )
    except ImportError:
        print(
    ...
    # Check if we have a 2D mesh (ring + ulysses)
    if hasattr(cp_mesh, "ndim") and cp_mesh.ndim == 2:
        set_rotate_method_2d(cp_rotate_method)
        set_load_balance_2d(cp_impl)
        # Use our custom 2D context parallel implementation
        assert (
            "upipe" in cp_impl or "usp" in cp_impl
        ), "2D context parallel only supports yunchang implementation"
        from torchtitan.distributed.context_parallel_2d import context_parallel_2d

        return context_parallel_2d(
            cp_mesh,
            buffers=cp_buffers,
        ...
        set_load_balance(cp_impl)
        from patch_torch_files._attention import context_parallel
