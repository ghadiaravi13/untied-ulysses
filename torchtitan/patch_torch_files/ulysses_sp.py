...
class UlyssesSPAttentionHF(torch.nn.Module):
    ...
    def __init__(
        ...
        if version.parse(transformers_version_have) < version.parse(
        ...
        if self.attn_head_count % self.world_size != 0:
            raise ValueError(
                f"Attention head count {attn_head_count} is not divisible by SP size {self.world_size}"
            )
        if not (
            self.global_kv_head_count % self.world_size == 0
            or self.world_size % self.global_kv_head_count == 0
        ):
            raise ValueError(
        ...
        self.required_query_shape = torch.Size(
            [local_seq_length, batch_size, attn_head_count, attn_head_size]
        )
        self.required_key_value_shape = torch.Size(
            [local_seq_length, batch_size, kv_head_count, attn_head_size]
        )
    ...
    def _combine_local_sequences(
        ...
        def combine_sequence(input, head_type):
            ...
            input = rearrange(
                input, "sl_l bs ws hc_l hs -> ws sl_l bs hc_l hs"
            ).contiguous()
            ...
            output = output.reshape(
                [self.global_seq_length, *output.shape[2:]]
            ).contiguous()
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if (
            self.seq_length_is_variable
            and current_local_seq_length != self.required_query_shape[0]
        ):
            ...
            self.required_query_shape = torch.Size(
                [self.local_seq_length] + list(self.required_query_shape)[1:]
            )
            self.required_key_value_shape = torch.Size(
                [self.local_seq_length] + list(self.required_key_value_shape)[1:]
            )
            self.required_context_shape = torch.Size(
                [self.global_seq_length] + list(self.required_context_shape)[1:]
            )
        ...
        if "position_ids" in kwargs:
            position_ids_list = [
                torch.empty_like(kwargs["position_ids"]) for _ in range(self.world_size)
            ]
            dist.all_gather(
        ...
        query_layer, key_layer, value_layer = self._combine_local_sequences(
            query, key, value
        )
        # returns: [sl bs hc_l hs]

        query_layer = rearrange(
            query_layer, "sl bs hc_l hs -> bs hc_l sl hs"
        ).contiguous()
        key_layer = rearrange(key_layer, "sl bs hc_l hs -> bs hc_l sl hs").contiguous()
        value_layer = rearrange(
            value_layer, "sl bs hc_l hs -> bs hc_l sl hs"
        ).contiguous()
        ...
        if not self.skip_all_but_last_attention_debug_mode:
            ...
            self.rotating_layer_counter = (
                self.rotating_layer_counter + 1
            ...
            if self.rotating_layer_counter % self.num_hidden_layers == 0:
                ...
                context_layer = rearrange(
                    query_layer, "bs hc_l sl ... -> bs sl hc_l ..."
                )
                attn_weights = None
...
class UlyssesSPDataLoaderAdapter:
    ...
    def refill(self):
        ...
        seqlen = torch.tensor(
            batch["input_ids"].shape[1], dtype=torch.int64, device=self.device
        )
        seqlens = [
        ...
        for batch in micro_batches.values():
            ...
            if seq_length % self.sp_world_size != 0:
                raise ValueError(
                    f"batch's seqlen={seq_length} isn't divisible by sp-size={self.sp_world_size}"
                )
            ...
            for k in batch.keys():
                ...
                batch[k] = batch[k][
                    :, chunk_len * self.sp_rank : chunk_len * (self.sp_rank + 1)
                ].cpu()
...
class SequenceTiledCompute(torch.autograd.Function):
    ...
    def forward(
        ...
        *args,
        ...
        with torch.no_grad():
            ...
            ctx.grad_requiring_tensor_key_index = (keys_to_shard + keys_to_pass).index(
                grad_requiring_tensor_key
            )
        ...
        with torch.no_grad():
            ...
            if output_unshard_dimension == 0:
                ...
                output_unsharded = torch.cat(
                    [l.unsqueeze(0) for l in output_shards],
                    dim=output_unshard_dimension,
                )
            else:
                output_unsharded = torch.cat(
                    output_shards, dim=output_unshard_dimension
                )  # .clone().detach()
            ...
            if output_reduction is None:
                ...
                raise ValueError(
                    f"unknown value {output_reduction}: valid values are: none/mean/sum"
                )
    ...
    def backward(ctx, *grads) -> torch.Tensor:
        ...
        kwargs_to_shard_shards = {
            k: list(torch.chunk(v, chunks=shards, dim=1))
            for k, v in kwargs_to_shard.items()
        }
        ...
        for i in range(shards):
            ...
            grad_requiring_tensor_shard = kwargs_to_shard_shard[
                grad_requiring_tensor_key
            ]

            grad_requiring_tensor_shard.requires_grad_(
                grad_requiring_tensor_requires_grad
            )

            # if seqlen is not exactly divisible by shards the last step will be shorter than shard_step
            shard_step = kwargs_to_shard_shards[grad_requiring_tensor_key][i].shape[1]
            shard_offset = (
                i * kwargs_to_shard_shards[grad_requiring_tensor_key][0].shape[1]
            )
            ...
            if output_unshard_dimension == 0:
                ...
                incoming_grad_shard = incoming_grad.narrow(
                    1, shard_offset, shard_step
                ).view_as(grad_requiring_tensor_shard)
            ...
            if grad_requiring_tensor.shape[0] > 1:
                ...
                grad_requiring_tensor_grad.narrow(1, shard_offset, shard_step).view_as(
                    grad_requiring_tensor_shard
                ).copy_(grad_requiring_tensor_shard.grad)
...
# @torch.library.custom_op("yunchang::_tiled_mlp_operator", mutates_args=(), device_types="cuda")
def tiled_mlp_operator(x: torch.Tensor, shards: int) -> torch.Tensor:
    global mlp_fn, self_obj
    x_shards = list(torch.chunk(x, chunks=shards, dim=1))
    with torch.no_grad():
        output_shards = [mlp_fn(self_obj, x_shard) for x_shard in x_shards]
    output_unsharded = torch.cat(output_shards, dim=1)
    return output_unsharded


# @tiled_mlp_operator.register_fake
# def _(x: torch.Tensor, shards: int) -> torch.Tensor:
...
class TiledMLP(torch.autograd.Function):
    ...
    def forward(
        ...
        output_unsharded = tiled_mlp_operator(x, shards)
...
class TiledFusedLogitsLoss(torch.autograd.Function):
    ...
    def forward(
        ...
        if output_reduction not in ["mean", "sum"]:
            raise ValueError(
                f'unknown reduction {output_reduction}: valid values are: "mean"/"sum"'
            )
        if x.dim() < 2:
        ...
        if mask is not None:
            if mask.dim() != 2:
                raise ValueError(
                    f"mask must be 2D [batch_size, seq_len], but got {mask.dim()}"
                )
...
class TiledLoss(torch.autograd.Function):
    ...
    def forward(ctx, loss_fn, logits, vocab_size, shift_labels, shards) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for i in range(shards):
                ...
                shift_labels_shard = shift_labels[
                    :, i * shard_step : (i + 1) * shard_step
                ]
    ...
    def backward(ctx, *grads) -> torch.Tensor:
        ...
        for i in range(shards):
            ...
            logits_shard.grad = logits_grad.narrow(1, shard_offset, shard_step).view_as(
                logits_shard
            )
...
class UlyssesSPFwdLossBwdWithLogits:
    def __init__(
        self, model, model_unwrapped, device, num_loss_logit_shards="auto", **kwargs
    ):
    ...
    def sp_fwd_loss_bwd(self, batch) -> torch.Tensor:
        ...
        for sub_step_id in range(self.sp_world_size):
            ...
            for rank in range(self.sp_world_size):
                non_skipped = (
                    (
                        batch["labels"][:, chunk_len * rank : chunk_len * (rank + 1)]
                        != -100
                    )
                    .sum()
                    .item()
            ...
            loss = sum(
                losses_per_rank[rank] * non_skipped_items[rank]
                for rank in range(self.sp_world_size)
            ) / sum(non_skipped_items.values())
