...
try:
    ...
    flash_attn_version = version.parse("3.0.0b1")
except ImportError:
        ...
        with torch.no_grad():
            ...
            for i in range(num_chunks):
                ...
                for k_i in range(len(global_k)):
                    causal_chunk = i == k_i
                    if flash_attn_version >= version.parse("2.6.0") and flash_attn_version < version.parse("3.0.0b1"):
                    ...
                    elif flash_attn_version >= version.parse("3.0.0b1"):
                        block_out, block_lse, *rest = _flash_attn_forward(q,
                                                                        k,
                                                                        v,
                ...
                for k_i in range(len(global_k)):
                    ...
                    with get_accelerator().stream(compute_stream):
                        if flash_attn_version >= version.parse("2.6.0") and flash_attn_version < version.parse("3.0.0b1"):
                            ...
                            block_out, block_lse, *rest = _flash_attn_forward(global_q[q_compute_chunk_idx].get_gpu_chunk(),
                                                                            global_k[kv_compute_chunk_idx].get_gpu_chunk(),
                                                                            global_v[kv_compute_chunk_idx].get_gpu_chunk(),
                                                                            None, None,  # k_new, v_new
                                                                            None,  # qv
...
class FPDT_Attention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.upipe = os.environ.get("UPIPE", "0") == "1"
    ...
    def forward(self,
        ...
        if not cpu_offloading or self.num_chunks_attn == 1:
            ...
            ulysses_degree = dist.get_world_size(self.spg)
            pipe_degree = self.config.num_attention_heads // ulysses_degree
            gqa_ratio = self.config.num_attention_heads // self.config.num_key_value_heads
            output = []
            for stage in range(pipe_degree):
                q_wt = self.qkv_linear_weight[stage*self.projection_size//pipe_degree:(stage+1)*self.projection_size//pipe_degree]
                q_bias = self.qkv_linear_bias[stage*self.projection_size//pipe_degree:(stage+1)*self.projection_size//pipe_degree]
                k_wt = self.qkv_linear_weight[self.projection_size + ((stage//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree): self.projection_size + (((stage+1)//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree)]
                k_bias = self.qkv_linear_bias[self.projection_size + ((stage//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree): self.projection_size + (((stage+1)//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree)]
                v_wt = self.qkv_linear_weight[self.projection_size + self.kv_projection_size + ((stage//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree): self.projection_size + self.kv_projection_size + (((stage+1)//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree)]
                v_bias = self.qkv_linear_bias[self.projection_size + self.kv_projection_size + ((stage//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree): self.projection_size + self.kv_projection_size + (((stage+1)//gqa_ratio)*self.hidden_size_per_attention_head*ulysses_degree)]

                qkv_wt = torch.cat([q_wt, k_wt, v_wt], dim=0)
                qkv_bias = torch.cat([q_bias, k_bias, v_bias], dim=0)

                output.append(_FPDTGPUOffloadingAttentionImpl_.apply(
                    layernorm_output, attention_mask, inference_params, rotary_pos_emb, self.spg, self.scatter_idx,
                    self.gather_idx, self.hidden_size, self.projection_size//pipe_degree, self.hidden_size_per_attention_head,
                    self.hidden_size_per_attention_head*ulysses_degree, qkv_wt, qkv_bias, self.dropout,
                    self.num_chunks_attn, cpu_offloading)
                )
...
class FPDT_FFN(torch.autograd.Function):
    ...
    def forward(ctx: Any, x, w1, b1, w2, b2, add_bias, chunk_size):
        ...
        with torch.no_grad():
            ...
            if os.environ.get("UPIPE", "0") == "1":
                num_chunk = x.shape[0] // x.shape[-1]
