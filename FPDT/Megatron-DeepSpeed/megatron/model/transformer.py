...
from megatron.patch.fpdt_layer import FPDT_FFN, FPDT_Attention
...
# Best-effort import for FlashAttention-3 (uses varlen-style interface in most builds)
try:
    from flash_attn_interface import FlashAttnFunc as flash_attn_v3_func #FlashAttnVarlenFunc
except ImportError:
...
class ParallelMLP(MegatronModule):
    ...
    def forward(self, hidden_states):
        ...
        if self.ds_sequence_parallel_fpdt:
            output, output_bias = FPDT_FFN.apply(hidden_states, self.dense_h_to_4h.weight, self.dense_h_to_4h.bias, self.dense_4h_to_h.weight, self.dense_4h_to_h.bias, self.add_bias, self.fpdt_FFN_chunk_size, self.swiglu)
        else:
            # [s, b, 4hp]
...
class FlashSelfAttention(torch.nn.Module):
    ...
    def __init__(self, causal=False, softmax_scale=None, attention_dropout=0.0,
        ...
        super().__init__()
        assert flash_attn_unpadded_func is not None or flash_attn_varlen_func is not None or flash_attn_builder is not None or flash_attn_v3_func is not None, \
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, config, layer_number,
        ...
        if self.use_flash_attn:
            ...
            if args.use_flash_attn_v3:
                assert flash_attn_v3_func != None, "Cannot import FlashAttention v3 "
...
def launch_chunk_attention(args, config):
    ...
    return FPDT_Attention(config,
                            ...
                            use_fa3=args.use_flash_attn_v3)
