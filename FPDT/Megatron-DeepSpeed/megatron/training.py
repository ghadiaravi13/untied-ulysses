...
from megatron.profile_utils import (
    create_torch_profiler,
    MemoryProfiler,
    MEMORY_SNAPSHOT_DUMP_FREQ,
)
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    # Create torch profiler if enabled via args
    torch_profiler = create_torch_profiler(args)

    # Create memory profiler if enabled — records allocation history
    # continuously and dumps snapshots every MEMORY_SNAPSHOT_DUMP_FREQ steps
    # (or immediately on OOM).
    memory_trace_enabled = getattr(args, 'torch_profiler_enable', False)
    memory_profiler = None
    if memory_trace_enabled:
        snapshot_dir = getattr(args, 'torch_profiler_trace_dir', None)
        if snapshot_dir is None:
            snapshot_dir = os.path.join(os.getcwd(), "profiler_traces")
        snapshot_dir = os.path.join(snapshot_dir, "memory_snapshots")
        memory_profiler = MemoryProfiler(
            snapshot_dir, step_num=args.iteration, freq=MEMORY_SNAPSHOT_DUMP_FREQ
        )
        print_rank_0(f"Memory profiler active. Snapshots will be saved at {snapshot_dir}")
    ...
    while iteration < args.train_iters and (args.train_tokens is None or \
        ...
        try:
            if torch_profiler is not None:
                print_rank_0("PyTorch Profiler enabled\n")
                with torch_profiler:
            ...
            if memory_profiler is not None:
                print_rank_0("CUDA OOM detected — dumping memory snapshot before re-raising")
                memory_profiler.step(exit_ctx=True)
            raise
        except RuntimeError as e:
            if "CUDA out of memory" in str(e) and memory_profiler is not None:
                print_rank_0("CUDA OOM (RuntimeError) detected — dumping memory snapshot before re-raising")
                memory_profiler.step(exit_ctx=True)
            raise

        # Periodic memory snapshot dump (no-op if not at a dump-frequency boundary)
        if memory_profiler is not None:
