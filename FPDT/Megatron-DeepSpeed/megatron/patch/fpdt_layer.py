...
try:
    from flash_attn.flash_attn_interface import _flash_attn_forward as _flash_attn_forward_fa2, _flash_attn_backward as _flash_attn_backward_fa2
except ImportError:
    raise ImportError("FPDT requires atleast Flash Attention 2")

# Check for Flash Attention 3 (FA3)
is_flash_attn_3 = False
try:
    # Try to import FA3 first (flash_attn_3 module)
    from flash_attn_interface import _flash_attn_forward as _flash_attn_forward_fa3
    from flash_attn_interface import _flash_attn_backward as _flash_attn_backward_fa3
    is_flash_attn_3 = True
    
except ImportError:
    warnings.warn("Flash Attention 3 not found, using Flash Attention 2")

from einops import rearrange
from deepspeed.sequence.layer import single_all_to_all, apply_rotary_pos_emb
        ...
        with torch.no_grad():
            ...
            for i in range(num_chunks):
                ...
                for k_i in range(len(global_k)):
                    causal_chunk = i == k_i
                    if use_fa3 and is_flash_attn_3:
                        # FA3 API: no dropout_p, no alibi_slopes, returns (out, softmax_lse, *rest)
                        block_out, block_lse, *_ = _flash_attn_forward_fa3(
                            global_q[i],
                            global_k[k_i],
                            global_v[k_i],
                            None,  # k_new
                            ...
                            ctx.softmax_scale,
                            causal_chunk,
                            ctx.window_size[0], #window_size_left  # NOTE: this might be different in some FA3 versions, ie, only window_size tuple instead of left and right
                            ctx.window_size[1], #window_size_right
                            0,     # attention_chunk
                            0.0,   # softcap
                        )
                    else:
                        block_out, block_lse, _, _ = _flash_attn_forward_fa2(
                            global_q[i],
                            global_k[k_i],
                            global_v[k_i],
                            ctx.dropout_p,
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(num_chunks):
            ...
            for q_i in range(num_chunks):
                ...
                if ctx.use_fa3 and is_flash_attn_3:
                    # FA3 API: no dropout_p, no alibi_slopes, uses window_size tuple
                    _flash_attn_backward_fa3(
                        ...
                        softmax_scale,
                        causal_chunk,
                        window_size[0], #window_size_left  # NOTE: this might be different in some FA3 versions, ie, only window_size tuple instead of left and right
        ...
        return torch.cat(
            ...
            dim=0).to(dtype), None, None, None, None, None, None, None, None, None, None, grad_qkv_linear_weight.to(
                dtype), grad_qkv_linear_bias.to(dtype), None, None, None, None
    ...
    def forward(self,
        ...
        if not cpu_offloading or self.num_chunks_attn == 1:
            ...
            output = _FPDTGPUOffloadingAttentionImpl_.apply(
                ...
                self.num_chunks_attn, cpu_offloading, self.use_fa3)
...
class FPDT_FFN(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx: Any, x, w1, b1, w2, b2, add_bias, chunk_size, swiglu=False):
        ...
        with torch.no_grad():
            ...
            for i in range(num_chunk):
                st = i * chunk_size
                ed = st + chunk_size
                x_ = torch.matmul(x[st:ed], w1.t())
                if b1 is not None:
                    x_ = x_ + b1
                if swiglu:
                    x_gate, x_up = torch.chunk(x_, 2, dim=-1)
                    x_ = torch.nn.functional.silu(x_gate) * x_up
                    del x_gate, x_up
                else:
                    x_ = bias_gelu(x_)
                if add_bias:
    ...
    def backward(ctx, grad_output, grad_bias):
        ...
        grad_b2 = torch.zeros(b2.shape, device=device, dtype=torch.float) if b2 is not None else None
        grad_w1 = torch.zeros(w1.shape, device=device, dtype=torch.float)
        grad_b1 = torch.zeros(b1.shape, device=device, dtype=torch.float) if b1 is not None else None
        ...
        for i in range(num_chunk):
            ...
            if swiglu:
                # Recompute forward: z = x @ w1.t() + b1, split into gate/up, y = silu(gate) * up
                z = torch.matmul(x_chunk, w1.t())
                if b1 is not None:
                    z = z + b1
                gate, up = torch.chunk(z, 2, dim=-1)
                sig_gate = torch.sigmoid(gate)
                silu_gate = gate * sig_gate  # silu(gate)
                activated = silu_gate * up   # output of activation: silu(gate) * up

                # grad_w2: grad_output^T @ activated
                grad_w2.add_(
                    torch.matmul(grad_output[st:ed].reshape(-1, grad_output.shape[2]).t(),
                                 activated.reshape(-1, activated.shape[2])))
                del activated

                # grad through w2: [grad_output @ w2] gives grad w.r.t. silu(gate)*up
                grad_act = torch.matmul(grad_output[st:ed], w2)  # shape: [..., ffn_hidden_size]

                # SwiGLU backward:
                # d/d(gate) = grad_act * up * silu'(gate)
                #   where silu'(gate) = sig(gate) * (1 + gate * (1 - sig(gate)))
                # d/d(up) = grad_act * silu(gate)
                silu_prime = sig_gate * (1 + gate * (1 - sig_gate))
                grad_gate = grad_act * up * silu_prime
                grad_up = grad_act * silu_gate
                del sig_gate, silu_gate, silu_prime, gate, up, grad_act

                # Reassemble gradient w.r.t. z (the pre-split tensor)
                grad_z = torch.cat([grad_gate, grad_up], dim=-1)
                del grad_gate, grad_up

                # grad_w1: grad_z^T @ x_chunk
                grad_w1.add_(torch.matmul(
                    grad_z.reshape(-1, grad_z.shape[2]).t(), x_chunk.reshape(-1, x.shape[2])))
                if b1 is not None:
                    grad_b1.add_(grad_z.sum(0).sum(0))

                # grad_x: grad_z @ w1
                x[st:ed].copy_(torch.matmul(grad_z, w1))
                del grad_z, z
        ...
        return x, grad_w1.to(dtype), grad_b1.to(dtype) if grad_b1 is not None else None, grad_w2.to(dtype), grad_b2.to(dtype) if grad_b2 is not None else None, None, None, None
