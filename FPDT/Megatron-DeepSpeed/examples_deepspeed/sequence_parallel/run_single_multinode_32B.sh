...
# has sensible defaults for a 2×8-GPU H100 setup.
#
...
#     MASTER_ADDR=my-master-node NODE_RANK=0 bash run_multi_qwen3_32B_fpdt.sh
#
#   On the worker node (my-worker-node):
#     MASTER_ADDR=my-master-node NODE_RANK=1 bash run_multi_qwen3_32B_fpdt.sh
#
...
# Output directory
output_home="outputs/output_32B_FA3_multinode"
...
# Sequence parallelism (FPDT) — all 16 GPUs for maximum sequence length
sp_size=${SP_SIZE:-16}
...
# Data parallel size (computed — should be 1 with sp_size=16, dp disabled)
